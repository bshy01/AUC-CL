...
def train(net, data_loader, optimizer, loss_type, loss_fn=None):
    net.train()
...
if __name__ == '__main__':
    ...
    # supervised일 때
    if loss_type == 'supervised':
        train_data = utils.CIFAR10Single(root='./data/', train=True,
    ...
    memory_data = utils.CIFAR10Single(root='./data/', train=True, transform=utils.test_transform, download=True)
    test_data = utils.CIFAR10Single(root='./data/', train=False, transform=utils.test_transform, download=True)
    ...
    for epoch in range(1, epochs + 1):
        train_loss = train(model, train_loader, optimizer, loss_type=loss_type, loss_fn=loss_fn)
